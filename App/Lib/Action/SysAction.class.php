...
class SysAction extends CommonAction{
	...
	public function index(){
		...
		for($i=9;$i>-1;$i--){ //最近几天？ i 
			...
			$et = $bt+86400;
			$orders = M('user_list')->where(array('addtime'=>array('between',array($bt,$et))))->count(); //此处获取具体统计数量
			$data1 .= $orders.',';
			//$data2 .= $checkin.',';
		}
        ...
        $this->users = M('user_list')->count();
        $this->drive = M('car_drivelist')->count();
        $this->travel = M('car_xszlist')->count();
        $this->wts = M('car_wtslist')->count();
		$this->display();
	}
	...
	public function advs(){
		...
		if(IS_GET){
	 		$this->title = '轮播广告';
	 		$this->eq = '拼车管理';
	 		$list = M('sys_advs')->where(array())->order('px desc,addtime desc')->select();
	 		$this->assign('list',$list);
	 		$this->display();
	 	}else{
	 		$id = I('id');
	 		$t = I('t');
	 		if($t == -1){
	 			$info = M('sys_advs')->where(array('id'=>$id))->find();
	 			$r = M('sys_advs')->where(array('id'=>$id))->delete();
	 			if($r) @unlink($info['pic']);
	 		}else{
	 			$r = M('sys_advs')->where(array('id'=>$id))->setField('status',$t);
	 		}
	 		if($r) json('操作成功!',1);
	 		json('操作失败!');
	 	}
	...
	#添加修改轮播图#
 	public function AddAdvs(){
 		$this->IsAdm(true);
 		$data = I('post.');
 		if(!$data['pic']) json('请上传图片!');
 		if($data['id']){
 			$r = M('sys_advs')->where(array('id'=>$data['id']))->save($data);
 		}else{
 			$data['addtime'] = NOW_TIME;
 			$r = M('sys_advs')->add($data);
 		}
 		if($r){
 			json('保存成功!',1);
 		}else{
	...
	public function EditPrice(){
		...
		if(IS_GET){
			...
			$file='./App/Conf/price.php';
			//检测文件是否可写
			if (!is_writeable($file)) alert('配置文件无法写入！');
			$config	=	postdata();
			unset($config['files']);
			arr2file('./App/Conf/price.php',$config);
			@unlink('./App/Runtime/~runtime.php'); //更新缓存
			alert('修改成功!');
		}
	}
