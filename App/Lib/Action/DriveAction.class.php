...
class DriveAction extends MiniAction
{
	...
	public function myorder(){
		$this->IsCached();
		$list = $this->GetOrder('car_drivelist',$map);
	...
	public function GetPost(){
		...
		$id = I('id');
		$info = M('car_drivelist')->where(array('id'=>$id))->find();
	...
	public function GetPay(){
		$this->IsCached();
		$id  = I('get.id');
		$order = M('car_drivelist')->where(array('id'=>$id,'status'=>1,'openid'=>$this->openid))->find();
		if(!$order) exit(json_encode(array('code'=>0,'msg'=>'订单信息不存在')));
		$pay = $this->PayConf($order['orderid'],$order['fee'],'补换驾驶证','Drive/OrderDone',U('Wx/PayCallBack','','','',true));
		exit($pay);
	}
	...
	public function OrderDone($log){
		...
		$info = M('car_drivelist')->where(array('orderid'=>$log['out_trade_no']))->find();
	...
	public function OrderPaied($id,$log = false){
		...
		$data['status'] = 2;
		$r = M('car_drivelist')->where(array('id'=>$id))->save($data);
		$info = M('car_drivelist')->where(array('id'=>$id))->find();
		$this->TplDriveOrder('补换驾驶证','新订单通知！',$info['name'].$info['cphm']);
		if($r) return true;
	...
	public function orderlist(){
		...
		if (IS_GET) {
			...
			->order('a.addtime desc')->where($map)->page($p.',15')->field("a.id,b.nickname,a.name,a.tel,a.adr,a.cphm,a.status,a.addtime,a.paytime,b.headimgurl,a.fee,a.cardimg,a.sfz1,a.sfz2,a.video,a.openid")->select();
	...
	protected function OrderCancel($id,$type){
		...
		//更改状态
		$r = M('car_drivelist')->where(array('id'=>$id))->save(array('status'=>0,'donetime'=>NOW_TIME));
		//发送模版消息
		if($type != 1) $this->CanTplMsg('car_drivelist',$id,'补换驾驶证');	
		return $r;
	}
	...
	public function UserCancel(){
		$this->IsCached();
		$id = I('get.id');
		$info = M('car_drivelist')->where(array('id'=>$id,'openid'=>$this->openid))->find();
	...
	public function ConfirmOrder(){
		$this->IsCached();
		$id = I('get.id');
		$info = M('car_drivelist')->where(array('id'=>$id,'openid'=>$this->openid,'status'=>3))->find();
		if(!$info) exit(json_encode(array('code'=>0,'mag'=>'订单信息错误')));
		$r = M('car_drivelist')->where(array('id'=>$id,'openid'=>$this->openid))->setField('status',4);
		if($r) exit(json_encode(array('code'=>1)));
	...
	public function Post(){
		$this->IsAdm(true);
		$id = I('id');
		$info = M('car_drivelist')->where(array('id'=>$id,'status'=>2))->find();
		if(!$info) json('订单信息错误');
		$r = M('car_drivelist')->where(array('id'=>$id,'status'=>2))->save(array('express'=>I('express'),'expressnum'=>I('expressnum'),'status'=>3));
		//发送模版消息
		$post = array(
				'title'=>'补换驾驶证',
	...
	public function MakeOrderDone(){
		$this->IsAdm(true);
		$id = I('post.id');
		$info = M('car_drivelist')->where(array('id'=>$id,'status'=>3))->find();
		if(!$info) json('订单不存在');
		$r = M('car_drivelist')->where(array('id'=>$id,'status'=>3))->save(array('status'=>4,'donetime'=>NOW_TIME));
		if($r){
			json('操作成功',1);
		}else{
			json('操作失败');
