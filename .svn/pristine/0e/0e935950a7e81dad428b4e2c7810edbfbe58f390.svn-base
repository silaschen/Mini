...
if(!isNaN('<?php echo ($eq); ?>')){
  ...
  <div class="box box-solid">
    ...
    <div class="box-body">
      <form method="GET" action="<?php echo U('Inspect/orderlist');?>" id='search' style="margin-bottom:15px;">
      <div class="row">
        ...
        <div class="col-xs-6 col-md-6">
          <div class="input-group">
            ...
            <?php if($_GET['type'] != '' OR $_GET['word'] != '' OR $_GET['status'] != ''): ?><a title='清除条件' class="input-group-addon" href="<?php echo U('Inspect/orderlist');?>"><i class="fa fa-remove"></i></a><?php endif; ?>
          </div>
        </div>
      ...
      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
          ...
          <td><?php if($vo["sfz1"] != ''): ?><img src="<?php echo ($vo['sfz1']); ?>" width="35" onclick="ShowImg('<?php echo ($vo['sfz1']); ?>')"><?php endif; ?></td>
           <td><?php if($vo["sfz2"] != ''): ?><img src="<?php echo ($vo['sfz2']); ?>" width="35" onclick="ShowImg('<?php echo ($vo['sfz2']); ?>')"><?php endif; ?></td>
...
function DoneOrder(id){
    if(confirm('您确认要完成此订单吗?') == true){
      loading2('操作中...');
      $.post("<?php echo U('Common/MakeOrderDone');?>",{'table':'car_wtslist','id':id},function(data){
          alert(data.msg);
          if(data.ret == 1){
              location.reload();
          }else{
...
<script type="text/javascript">
   ...
   function Make(){
      ...
      loading2('操作中...');
      $.post("<?php echo U('Travel/PostNow');?>",{'id':id,'express':express,'expressnum':expressnum,'type':1},function(data){
            alert(data.msg);
            if(data.ret == 1){
              location.reload();
            }else{
