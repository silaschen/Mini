...
if(!isNaN('<?php echo ($eq); ?>')){
  ...
  <div class="box box-solid">
    ...
    <div class="box-body">
      <form method="GET" action="<?php echo U('RePlate/orderlist');?>" id='search' style="margin-bottom:15px;">
      <div class="row">
        ...
        <div class="col-xs-6 col-md-6">
          <div class="input-group">
            ...
            <?php if($_GET['type'] != '' OR $_GET['word'] != '' OR $_GET['status'] != ''): ?><a title='清除条件' class="input-group-addon" href="<?php echo U('RePlate/orderlist');?>"><i class="fa fa-remove"></i></a><?php endif; ?>
          </div>
        </div>
      </div>
      </form>
...
function DoneOrder(id){
    if(confirm('您确认要完成此订单吗?') == true){
      loading2('操作中...');
      $.post("<?php echo U('Common/MakeOrderDone');?>",{'table':'car_cpzlist','id':id},function(data){
          alert(data.msg);
          if(data.ret == 1){
              location.reload();
          }else{
...
<script type="text/javascript">
   ...
   function Make(){
      ...
      loading2('操作中...');
      $.post("<?php echo U('Common/PostNow');?>",{'id':id,'express':express,'expressnum':expressnum,'type':4},function(data){
